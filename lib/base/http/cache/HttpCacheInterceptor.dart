...
class HttpCacheInterceptor extends Interceptor {
  ...
  void onRequest(
    ...
    // 获取缓存模式
    CacheMode cacheMode =
        options.extra[CacheStrategy.CACHE_MODE] ?? CacheMode.ONLY_NETWORK;
    ...
    if (CacheMode.READ_CACHE_NETWORK_PUT == cacheMode) {
      ...
      if (cache == null || cache.expireTime <= DateTime.now().millisecondsSinceEpoch) {
        // 如果没有缓存或者缓存过期了
        return super.onRequest(options, handler);
      }
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) async {
    ...
    // 如果缓存模式是优先缓存，则先校验缓存
    await CacheManager.getInstance().putCacheWithResp(response, cacheConfig.defaultCacheTime);
    return super.onResponse(response, handler);
  }
